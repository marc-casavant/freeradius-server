...
(.venv) freeradius-multi-server % TEST_LOGGER_CONFIG=linelog_file DATA_PATH=$HOME/sandbox/multi-server-tests/environments/configs LISTENER_DIR=$HOME/freeradius-listener-logs docker compose -p custom_test-env-loadgen-5hs -f $HOME/sandbox/multi-server-tests/environments/docker-compose/env-loadgen-5hs.yml up
```
...
```bash
DATA_PATH=$HOME/sandbox/multi-server-tests/environments/configs make test-framework-custom-config-path -- -x -vvvv --compose $HOME/sandbox/multi-server-tests/environments/docker-compose/env-loadgen-5hs.yml --test $HOME/sandbox/multi-server-tests/tests/test-5hs-autoaccept.yml --use-files --listener-dir $HOME/freeradius-listener-logs
```